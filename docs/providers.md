...
When `supports_idle` is true, implement `idle(duration, strength)`. Idle support requires rig support. When `supports_locomotion` is true, implement `locomotion(duration, strength)`; locomotion support also requires rig support. When `supports_run` is true, implement `run(duration, strength)`; Run support also requires rig support. Animation tracks use rest-armature-space axes and seconds/radians; anatomy and motion design belong with the provider rather than shared Blender workflow code.
