...
`tests/blender/test_workflow.py` includes a test-only non-humanoid rotor that exercises shared rigid rig/animation behavior. Human 1.0 separately exercises weighted deformation, UV, generated-material, and generated-image-texture paths. These tests protect shared architecture boundaries without claiming arbitrary providers are automatically production-ready.
