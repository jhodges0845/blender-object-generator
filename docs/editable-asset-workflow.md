...
The first imported-component execution slice supports one artist-selected external Blender mesh at a time as a rigid Hair, Clothing, or Accessory component. Adoption is explicit ownership transfer rather than a copy: Asset Assistant creates the component root and portable record, reparents the existing mesh while preserving its world transform, and then immediately reuses the existing component inspection contract. The adopted mesh therefore enters the same validated remove lifecycle as generated rigid components. Root and `bone:<bone-name>` attachment targets are supported. Invalid attachment or inspection rolls back parenting and registry metadata.

Because adoption transfers ownership, removing the adopted component may delete the adopted mesh object. Asset Assistant does not silently claim arbitrary scene objects, body geometry, or already-owned component geometry. Parent-rig skinned imported adoption remains the next extension of this slice before imported component adoption is considered complete.
...
3. [ ] Add imported component registration/adoption so artist-authored reusable hair/clothing/accessory assets can enter the existing component lifecycle without being generated by a provider. Rigid mesh adoption is implemented; parent-rig skinned adoption remains before this item is complete.
