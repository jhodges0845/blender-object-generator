...
class AvianGeneratedMaterialTests(unittest.TestCase):
    def setUp(self):
    ...
    def _avian(self):
        ...
        root = create_asset(provider.mesh(values), name="MaterialAvian")
        root["object_type"] = provider.key
        for key, value in values.items():
            root[key] = value
        return root, provider, values

    def test_prepare_uses_portable_avian_plumage_and_texture(self):
    ...
    def test_prepare_preserves_existing_artist_material(self):
        root, _provider, _values = self._avian()
        mesh = next(obj for obj in root.children if obj.type == "MESH")
        artist = bpy.data.materials.new("Artist Avian Plumage")
